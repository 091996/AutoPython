...
logging.basicConfig(filename='test.log', level=logging.INFO, format=log_format,
                    datefmt=time_format)  # log名字，等级设定，log默认生成在该py文件所在目录
...
start_button = tk.Button(window, text="Start", font=('Arial', 20), command=lambda: hit("start")).place(x=100,
                                                                                                       y=100)  # 设置开始button
stop_button = tk.Button(window, text="Stop", font=('Arial', 20), command=lambda: hit("stop")).place(x=200,
                                                                                                    y=100)  # 设置停止button
